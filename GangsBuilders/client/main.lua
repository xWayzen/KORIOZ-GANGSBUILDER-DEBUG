...
function OpenVehicleSpawnerMenu()
	...
	ESX.TriggerServerCallback('esx_society:getVehiclesInGarage1', function(vehicles)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner', {
			...
			TriggerServerEvent('esx_society:removeVehicleFromGarage1', ActualGang.Name, vehicleProps)
		end, function(data, menu)
...
function OpenBodySearchMenu(player)
	ESX.TriggerServerCallback('GangsBuilderJob:getOtherPlayerData', function(data)
		if data.foundPlayer then
			...
			table.insert(elements, {label = '--- Argent & Argent Sale ---', value = nil})
			...
			for i = 1, #data.accounts, 1 do
				...
				if data.accounts[i].name == 'money' then
					table.insert(elements, {
						label = _U('confiscate_money') .. ' $' .. data.accounts[i].money,
						value = 'money',
						--itemType = 'item_account',
						amount = data.accounts[i].money
					})
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'body_search', {
				...
				if data2.current.value ~= nil then
					--ExecuteCommand("me A confisqué un objet / Armes")
					menu2.close()
...
AddEventHandler('GangsBuilderJob:OutVehicle', function(t)
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlPressed(0, 56) and PlayerData.job2 ~= nil and ActualGang and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'gang_actions') and (GetGameTimer() - GUI.Time) > 150 then
			OpenGangActionsMenu()
			GUI.Time = GetGameTimer()
		end
	end
...
AddEventHandler('GangsBuilderJob:drag', function(cop)
