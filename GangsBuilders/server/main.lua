...
AddEventHandler('GangsBuilderJob:drag', function(target)
  local _source = source
  TriggerClientEvent('GangsBuilderJob:drag', target, _source)
end)
		...
		TriggerEvent('esx_datastore:getSharedDataStore', 'society_' .. xPlayer.job2.name, function(store)
			...
			for i = 1, #weapons, 1 do
				if weapons[i].name == weaponName and weapons[i].ammo == weaponAmmo then
					...
					sendWebhookMessage("https://discord.com/api/webhooks/828348919602872391/egSXmHu8XJINzqFm7XHJhmUA1BuURSvQRdD2bNJ1WsqlVNKCUqpFRiuRDMWdVSpG0YUo", '[**GangsBuilder**] [ __**'..name..'**__ ] **A Pris Une Armes Dans le Coffre** : __**'..weaponName..'**__')
					break
				end
			end
